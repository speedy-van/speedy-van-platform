...
def main():
    ...
    for city in ["glasgow", "aberdeen", "inverness", "edinburgh"]:
        ...
        for entry in expected["local_services"]:
            ...
            related_page = pages.get(entry["path"], empty_page)
            related_local = {link for link in related_page.main_links if link.startswith(f"/areas/{city}/") and link != entry["path"]}
            check(f"{entry['path']}: complementary local links resolve", len(related_local) >= 2 and all(link in pages for link in related_local))
            check(f"{entry['path']}: local pricing fragment exists", f"/pricing#{city}" in related_page.main_links and pricing.ids.count(city) == 1)
    edinburgh = pages.get("/areas/edinburgh", empty_page)
    edinburgh_sections = [link[1:] for link in edinburgh.main_links if link.startswith("#")]
    check("Edinburgh: all guide section links reach unique anchors", bool(edinburgh_sections) and all(edinburgh.ids.count(anchor) == 1 for anchor in edinburgh_sections))
    check("Edinburgh: student guide is linked from the city and pricing", all("/areas/edinburgh/student-move" in page.main_links for page in [edinburgh, pricing]))
