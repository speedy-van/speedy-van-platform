    ...
    <div className={`flex flex-col gap-0.5 ${isOwn ? "items-end" : "items-start"}`}>
      <div className="flex items-center gap-1.5 px-1">
        ...
        {sender.role === "ADMIN" && !isOwn && (
          <span className="text-[10px] bg-primary-100 text-primary-700 rounded px-1 font-semibold">
            Support
          </span>
        )}
        {sender.role === "DRIVER" && !isOwn && (
          <span className="text-[10px] bg-amber-100 text-amber-800 rounded px-1 font-semibold">
            Driver
          </span>
        )}
      </div>
      <div
        className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed ${
          isOwn
            ...
            : "bg-slate-100 text-stone-950 rounded-tl-sm"
        }`}
      >
        {body}
      </div>
