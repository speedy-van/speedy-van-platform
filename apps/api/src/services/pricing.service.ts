...
async function loadConfig(): Promise<Record<string, Record<string, number>>> {
  ...
  const rows = await db.pricingConfig.findMany().catch(() => null);

  if (!rows) {
    // DB unavailable — pricing continues with built-in defaults from DEFAULT_PRICING_CONFIG
    return {};
  }
...
export async function calculatePrice(input: PricingCalculateInput): Promise<PricingResult> {
  ...
  // 28-day x 3-slot price calendar (T4)
